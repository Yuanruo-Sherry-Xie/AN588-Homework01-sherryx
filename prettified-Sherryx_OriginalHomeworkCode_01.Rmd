...
#install.packages("stringr") I ALREADY HAD THIS IN MY ENVIRONMENT. DONT HAVE TO REDOWNLOAD EVERY SINGLE TIME
library(stringr)

```

# Challenges from Module 4
...
# This make sure the vectors that gets picked out (the elements) does not have punctuation following them 
line_clean <- gsub("[[:punct:]]", "", line)

# easily divide a single string into a vector of substrings = a vector of words
#Christian's suggestion/question. I added the [[1]] so that the first element from the list, which is the vector of words, is extracted. This saves me the step of getting an output of varaibles/characters and then converting it to charcater vector. 
...
# Christian's suggestion: I left rows blank because I wanted to select everything within the rows with the 2nd, 3rd, and 6th columns
# If I did not leave the rows blank, I would put in 1-8 for all the rows
# column numbers (c(2, 3, 6)) means I concentrated the specific columns I want to select from 
x <- m[, c(2, 3, 6)]
x
...
```{r}
#I left columns blank because I wanted to select everything within the columns with the 6th to 8th row
# If I did not leave the columns blank, I would put in 1-10 for all the columns
...
```{r}
a[1, 1, 1, 2]
# basically telling the matrix to narrow in on row 1, column 1, in table 1,2 
# the only solution is = 300
```
...
```{r}
a[2, 3, 2, ]
# answers = 364, 264, 164,  64
#row 2, cloumn 3, in dimension 3 with a 2. there are multiple values because there are only 3 instructions
```
...
#I tried to start from the lower levels and go upwards, concentrate first
...
#Then list the bigger groups out: Strepsirhini
Strepsirhini <- list(Lemuroidea, Lorisoidea)
names(Strepsirhini) <- list("Lemuroidea", "Lorisoidea")
Strepsirhini

#Then list the bigger groups out: Anthropoidea
Anthropoidea <- list(Platyrrhini, Catarrhini)
names(Anthropoidea) <- list("Platyrrhini", "Catarrhini")
Anthropoidea

#Then list the bigger groups out: Haplorhini
...
```{r}
df <- read.csv("/Users/sherryxie/CODE/Github/repos/Homework/AN588-Homework1-sherryx/random-people.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
